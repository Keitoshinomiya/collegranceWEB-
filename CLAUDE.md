...
4. **販売価格**: `sellPrice = ceil((卸値 × 1.25 × 1.10 + 700) / 10) × 10`
   - 卸値×1.25+¥700が¥3,000未満なら `inStock: false` で除外（送料込み最安¥3,000ライン）
5. **商品画像取得**（`fetch-product-images.py --id {新ID}`）
   - fetch-product-images.py は自動で `.jpg` + `.webp` 両方を生成する（2026-05-13〜）
6. **画像品質チェック**（`validate-images.py --id {新ID}`）
7. **WebP補完**（既存画像をfetchし直さない場合）: `python3 convert-images-to-webp.py`
   - 未変換の `.jpg/.png` のみ `.webp` 生成、既存はスキップ（冪等）
8. **Keitoに画像目視確認依頼**（特にEDT/EDP瓶）
9. catalog_full.json再生成
...
### 在庫更新時のチェック順序
1. メイクアップExcel読み込み
2. 香水カテゴリ・**送料込み¥3,000以上**でフィルタ（卸値×1.25+¥700 ≥ ¥3,000）
3. 重複チェック → Slack通知（必要に応じて）
4. 価格計算: `sellPrice = ceil((卸値 × 1.25 × 1.10 + 700) / 10) × 10`（送料¥700内包・10円切上）
5. 価格5重チェック（`python check-prices.py`）
6. 画像取得（`python fetch-product-images.py --id {新ID}`、.jpg+.webp自動生成）
7. WebP補完（既存画像のみ、新規取得しない場合）: `python3 convert-images-to-webp.py`
8. 画像品質チェック（`python validate-images.py`）
9. Keitoに目視確認依頼（特にEDT/EDP瓶の違い）

### 送料・価格ルール（2026-05-13〜）
- **全フルボトル: 送料無料**（商品価格に¥700内包）
- カート画面 / Stripe Checkout: 送料行は「無料」表示
- products.json の `sellPrice` は送料込みの税込価格
- 新商品追加時もこのルールに従う（自動: `check-makeup-email.py` の MIN_PRICE=3000）
...
| `fetch-product-images.py` | Web画像取得 + rembg背景除去 + .webp自動生成 |
| `validate-images.py` | 商品画像の品質チェック + 自動再生成 |
| `convert-images-to-webp.py` | 既存画像の.webp一括変換（冪等、未変換のみ）|
...
【賞品構成】100人中30人が大きい当選 + 残り70人もベース特典で全員勝者
  🏆 1/100  : フルボトル ¥10,000 OFF（大当たり）
  🥈 5/100  : フルボトル ¥3,000 OFF（中当たり）
  🥉 24/100 : フルボトル ¥1,000 OFF（小当たり）
  🎁 70/100 : フルボトル ¥500 OFF（ベース特典・全員もらえる）

費用試算（100人参加あたり）:
  ¥10,000 + ¥15,000 + ¥24,000 + ¥35,000 = ¥84,000 / 100 = ¥840/人平均
  LTV ¥1,670 - クーポン費用 ¥840 = 純LTV ¥830/人 → 黒字確保
...
### AI診断クーポンの2フェーズ戦略
```
Phase 1（現在〜LINE Harness完成まで）:
  AI診断完了 → 即時クーポン発行（10% OFF / 24h / max_redemptions=1）
  - 摩擦最小、CVR優先
  - Stripe API（create-diagnosis-coupon Function）で動的発行
  - GA4イベント: ai_diagnosis_complete → diagnosis_coupon_issued/copied

Phase 2（LINE Harness完成後）:
  AI診断完了 → 「LINEで受け取る」のみ
  - 全クーポン導線をLINEに集約 → 友だちリスト最大化
  - Harnessのシナリオ配信でリマインド・LTV最大化
  - 香りくじ施策と統合運用

切り替えタイミング: line-harness-oss プロジェクトの本番稼働時
```
