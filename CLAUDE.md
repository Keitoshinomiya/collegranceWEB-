...
- **Anthropic API**: AI診断用（Netlify環境変数 `ANTHROPIC_API_KEY`）
...
| `stripe-setup.py` | Stripe商品一括登録 |
| `generate-links.py` | チャネル別UTM付きAmazonリンク生成 |
| `netlify/functions/ai-diagnosis.js` | AI香り診断API |
| `attribution-config.json` | Amazon Attributionタグ設定 |
...
```
SHOP: すべて | ブランド別 | 人気 | 小分け | フルボトル
ABOUT: ブランドストーリー | 香り診断
JOURNAL: 新着記事 | レビュー
SUPPORT: お問い合わせ | 配送・返品 | 特商法
```
