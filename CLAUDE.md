...
### ビジネスモデル
```
Amazon小分け（広告費100万/月）→ LINE登録（5%、1,300人）→ CRM育成 → フルボトル自社EC
                                                              → クロスセル（別の小分け）
空き瓶 → 3Dプリンタ → ナイトライト → TikTok動画 → 認知拡大（ループ）
```

### 販売チャネル
| チャネル | 商品 | 役割 |
|---|---|---|
| Amazon | 小分け+ケース | 広告で集客+マネタイズ（月間広告費約100万） |
| TikTok Shop | 小分け+ケース+ナイトライト（予定） | コンテンツ→販売 |
| メルカリShop | 小分け+ケース+ナイトライト（予定） | 販売チャネル |
| 自社EC(Stripe) | フルボトル+ナイトライト（予定） | 高利益率の本命 |

### LINE登録の導線
Amazon購入 → パッケージ内QR → 「アンケート回答でケースプレゼント」→ LINE登録（率5%）

### SNS
| チャネル | 担当 | 状態 | 役割 |
|---|---|---|---|
| TikTok(@collegrance) | みのべさん | 稼働中（毎日1本） | 小分け露出→TikTok Shop |
| Threads | keito（中の人） | 準備中 | 認知拡大→サイト流入 |
| Instagram | keito | 準備中 | 世界観構築→AI診断誘導 |

### サイト流入実績（直近30日、モバイルのみ）
- LINE(Lステップ): 70% / Google検索: 16% / Direct: 8%
- 総セッション: 585 / ユーザー: 441

### ナイトライト事業（新規）
- 3Dプリンタ × フルボトル空き瓶 → ナイトライト製作
- 製作動画 → TikTokコンテンツ（サステナブル路線）
- 販売: TikTok Shop / メルカリShop / 自社EC
- 香水の「試す→使う→残る→再生→広がる」全ライフサイクルをビジネス化

### 資料
- `COLLEGRANCE_事業全体像.pptx` — 事業説明PPT（11スライド）
