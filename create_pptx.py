#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""COLLEGRANCE 事業全体像 PowerPoint 生成スクリプト（14スライド版・最終版）"""
...
def slide_01_cover():
    ...
    add_textbox(slide, Inches(1), Inches(3.8), Inches(11), Inches(0.6),
                "事業全体像  2026年4月（最終版）", font_size=28, bold=True, color=COLOR_GREEN,
                alignment=PP_ALIGN.CENTER)
...
def slide_04_amazon_sales():
    ...
    lines = [
        ...
        ("", 8, False, COLOR_BODY),
        ("平均単価（税込）", 11, False, COLOR_GRAY),
        ("¥1,079", 22, True, COLOR_BLACK),
    ]
    add_multiline_textbox(slide, Inches(8.5), Inches(5.6), Inches(4), Inches(1.5), lines)
...
# スライド5: ユニットエコノミクス（新規）
# ══════════════════════════════════════════════════════════════
def slide_05_unit_economics():
    slide = prs.slides.add_slide(blank_layout)
    add_section_title(slide, "ユニットエコノミクス", "1個あたりの利益構造")

    # ── 左: Amazon小分け ──
    add_textbox(slide, Inches(0.5), Inches(1.4), Inches(4), Inches(0.3),
                "Amazon小分け（1.5ml）", font_size=18, bold=True, color=COLOR_GREEN)
    ...
    for i, (label, value, color, bold) in enumerate(econ_items):
        ...
        if label.startswith("="):
            # 区切り線
            sep = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE,
                                         Inches(0.8), y - Inches(0.05), Inches(3.5), Inches(0.02))
            sep.fill.solid()
            sep.fill.fore_color.rgb = COLOR_GREEN
            sep.line.fill.background()
        add_textbox(slide, Inches(0.8), y, Inches(2.5), Inches(0.3),
                    label, font_size=13, bold=bold, color=color)
        add_textbox(slide, Inches(3.3), y, Inches(1.2), Inches(0.3),
                    value, font_size=13, bold=bold, color=color, alignment=PP_ALIGN.RIGHT)

    # ケース付きの加算
    add_rect(slide, Inches(0.5), Inches(4.3), Inches(4.2), Inches(1.0), COLOR_GREEN_LIGHT,
    ...
    # ── 中央: フルボトル ──
    mx = Inches(5.2)
    add_textbox(slide, mx, Inches(1.4), Inches(3.5), Inches(0.3),
                "フルボトル（自社EC）", font_size=18, bold=True, color=COLOR_RED)

    fb_items = [
        ("価格設定", "仕入原価 × 1.25 × 1.10", COLOR_BODY),
        ("粗利率", "25%（仕入原価に対して）", COLOR_RED),
        ("在庫リスク", "ゼロ（受注後発注モデル）", COLOR_GREEN),
        ("広告費", "¥0", COLOR_GREEN),
        ("Amazon手数料", "¥0", COLOR_GREEN),
    ]
    for i, (label, value, color) in enumerate(fb_items):
        y = Inches(1.9) + i * Inches(0.5)
        add_textbox(slide, mx, y, Inches(1.5), Inches(0.3),
                    label, font_size=12, bold=True, color=COLOR_GRAY)
        add_textbox(slide, mx + Inches(1.5), y, Inches(2.5), Inches(0.3),
                    value, font_size=12, color=color)

    add_rect(slide, mx, Inches(4.3), Inches(3.5), Inches(1.0), COLOR_RED,
             text="低マージンだが在庫リスクゼロ\nLINE経由でAmazon広告費不要", font_size=12,
             font_color=COLOR_WHITE, bold=True)

    # ── 右: ナイトライト ──
    rx = Inches(9.2)
    add_textbox(slide, rx, Inches(1.4), Inches(3.5), Inches(0.3),
                "ナイトライト（新規）", font_size=18, bold=True, color=COLOR_ORANGE)
    ...
    for i, (label, value) in enumerate(nl_items):
        y = Inches(1.9) + i * Inches(0.45)
        add_textbox(slide, rx, y, Inches(2.0), Inches(0.3),
                    label, font_size=12, bold=True if value else False,
                    color=COLOR_GRAY if not value else COLOR_BODY)
        if value:
            add_textbox(slide, rx + Inches(2.0), y, Inches(1.5), Inches(0.3),
                        value, font_size=12, bold=True,
                        color=COLOR_ORANGE if label == "利益率" else COLOR_BLACK,
                        alignment=PP_ALIGN.RIGHT)

    add_rect(slide, rx, Inches(4.3), Inches(3.5), Inches(1.0), COLOR_ORANGE,
             text="超高利益率\n空き瓶リユースでコストほぼゼロ", font_size=12,
             font_color=COLOR_WHITE, bold=True)

    # 下部サマリー
    add_rect(slide, Inches(0.5), Inches(5.7), Inches(12.3), Inches(0.8), COLOR_GREEN_LIGHT,
             font_size=10, font_color=COLOR_BLACK)
    shape = slide.shapes[-1]
    tf = shape.text_frame
...
def slide_09_channels():
    ...
    data = [
        ...
        [("Amazon", True, COLOR_BODY), ("小分け+ケース", False, COLOR_BODY),
         ("集客+販売（広告費~75万/月）", False, COLOR_BODY), ("稼働中", True, COLOR_GREEN)],
        [("TikTok Shop", True, COLOR_BODY), ("小分け+ケース", False, COLOR_BODY),
         ("コンテンツ → 販売", False, COLOR_BODY), ("稼働中（25件/月）", True, COLOR_GREEN)],
...
def slide_12_strategy():
    slide = prs.slides.add_slide(blank_layout)
    add_section_title(slide, "現在の戦略（クロスセル/アップセル）",
...
# ══════════════════════════════════════════════════════════════
# スライド13: 今期目標シミュレーション（新規）
# ══════════════════════════════════════════════════════════════
def slide_13_target_simulation():
    slide = prs.slides.add_slide(blank_layout)
    add_section_title(slide, "2026年度 売上目標シミュレーション", "目標: 5,000万円")

    # 左側: チャネル別内訳テーブル
    add_textbox(slide, Inches(0.5), Inches(1.4), Inches(6), Inches(0.3),
                "チャネル別売上シミュレーション", font_size=16, bold=True, color=COLOR_GREEN)

    sim_data = [
        [("チャネル", True, COLOR_WHITE), ("計算根拠", True, COLOR_WHITE),
         ("年間売上", True, COLOR_WHITE)],
        [("Amazon小分け", True, COLOR_BODY),
         ("月平均¥380万 x 12ヶ月", False, COLOR_BODY),
         ("¥45,600,000", True, COLOR_BLACK)],
        [("フルボトル(LINE経由)", True, COLOR_RED),
         ("検討者728人 x 転換率20% x ¥12,000", False, COLOR_BODY),
         ("¥1,740,000", True, COLOR_RED)],
        [("クロスセル(別の小分け)", True, COLOR_GREEN_DARK),
         ("1,087人 x リピ率30% x ¥1,079", False, COLOR_BODY),
         ("¥351,754", True, COLOR_GREEN_DARK)],
        [("TikTok Shop", True, COLOR_BODY),
         ("SPS獲得後 月100件 x ¥1,079 x 12", False, COLOR_BODY),
         ("¥1,294,800", True, COLOR_BODY)],
        [("ナイトライト", True, COLOR_ORANGE),
         ("月30個 x ¥3,480 x 12ヶ月", False, COLOR_BODY),
         ("¥1,252,800", True, COLOR_ORANGE)],
        [("メルカリShop", True, COLOR_BODY),
         ("月50件 x ¥1,079 x 12", False, COLOR_BODY),
         ("¥647,400", True, COLOR_BODY)],
        [("合計", True, COLOR_BLACK),
         ("", False, COLOR_BODY),
         ("¥50,886,754", True, COLOR_GREEN)],
    ]

    add_table(slide, Inches(0.5), Inches(1.8), Inches(7.5), Inches(4.0),
              8, 3, sim_data,
              col_widths=[Inches(2.2), Inches(3.0), Inches(2.3)])

    # 右側: 横棒グラフ風の売上構成
    rx = Inches(8.5)
    add_textbox(slide, rx, Inches(1.4), Inches(4.5), Inches(0.3),
                "売上構成", font_size=16, bold=True, color=COLOR_GREEN)

    # チャネル別データ（横棒グラフ）
    bar_data = [
        ("Amazon小分け", 45600000, COLOR_GREEN),
        ("フルボトル", 1740000, COLOR_RED),
        ("クロスセル", 351754, COLOR_GREEN_DARK),
        ("TikTok Shop", 1294800, COLOR_BLUE),
        ("ナイトライト", 1252800, COLOR_ORANGE),
        ("メルカリ", 647400, COLOR_PURPLE),
    ]
    max_val = 45600000
    bar_max_w = Inches(4.0)
    bar_h = Inches(0.4)

    for i, (label, amount, color) in enumerate(bar_data):
        y = Inches(1.9) + i * Inches(0.6)
        add_textbox(slide, rx - Inches(0.2), y - Inches(0.2), Inches(1.5), Inches(0.2),
                    label, font_size=9, bold=True, color=COLOR_BODY)

        # 棒の長さ（対数的にスケール調整: Amazon以外が見えるように最低幅を設定）
        if amount >= 10000000:
            ratio = 1.0
        else:
            # 非Amazon: 最低0.1、最大0.25の範囲でスケール
            ratio = max(0.08, min(0.25, amount / max_val * 8))
        w = max(int(bar_max_w * ratio), Inches(0.3))
        add_rect(slide, rx, y, w, bar_h, color,
                 text=f"¥{amount:,.0f}", font_size=8, font_color=COLOR_WHITE, bold=True,
                 alignment=PP_ALIGN.LEFT)

    # 目標達成ライン
    add_rect(slide, Inches(0.5), Inches(6.0), Inches(12.3), Inches(0.7), COLOR_GREEN,
             font_size=10, font_color=COLOR_WHITE)
    shape = slide.shapes[-1]
    tf = shape.text_frame
    tf.paragraphs[0].text = "合計: ¥50,886,754 → 目標5,000万円 達成可能"
    ...
    tf.paragraphs[0].alignment = PP_ALIGN.CENTER
    p2 = tf.add_paragraph()
    p2.text = "※広告費の最適化（ROAS改善）による利益率向上も見込む"
    p2.font.size = Pt(11)
...
if __name__ == "__main__":
    slide_01_cover()        # 1. 表紙
    slide_02_overview()     # 2. 事業概要
    slide_03_business_model()  # 3. ビジネスモデル全体図
    slide_04_amazon_sales()    # 4. Amazon販売実績（実データ更新）
    slide_05_unit_economics()  # 5. ユニットエコノミクス（新規）
    slide_06_monthly_pl()      # 6. 月別P/L（新規）
    slide_07_funnel()          # 7. アンケートファネル分析
    slide_08_customer()        # 8. 顧客属性
    slide_09_channels()        # 9. 販売チャネル一覧
    slide_10_line_crm()        # 10. LINE CRM設計
    slide_11_site_sns()        # 11. 自社サイト + SNS戦略
    slide_12_strategy()        # 12. 現在の戦略（クロスセル/アップセル）
    slide_13_target_simulation()  # 13. 今期目標シミュレーション（新規）
    slide_14_roadmap()         # 14. ロードマップ
